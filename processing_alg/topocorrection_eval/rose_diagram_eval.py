...
class RoseDiagramEvalAlgorithm(TopoCorrectionEvalAlgorithm):

    @staticmethod
    def get_name():
        return "Rose diagrams"

    def process_internal(self) -> Any:
        slope_path = self.ctx.calculate_slope(in_radians=False)
        if self.ctx.qgis_feedback.isCanceled():
            return {}

        aspect_path = self.ctx.calculate_aspect(in_radians=False)
        if self.ctx.qgis_feedback.isCanceled():
            return {}

        build_polar_diagrams(
            self.ctx.input_layer.source(),
            slope_path,
            aspect_path,
